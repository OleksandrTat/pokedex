...
// Функція для отримання дерева еволюції покемона
function getEvolutionTree($conn, $pokemonId) {
    try {
        // Створюємо запит на виклик збереженої процедури
        $stmt = $conn->prepare("CALL evolution_tree_pokemon(?)");
        ...
        if ($result && $row = $result->fetch_assoc()) {
            return $row['result']; // Повертаємо результат процедури
        }
...
if (isset($_POST['action']) && $_POST['action'] == 'getPokemon') {
    ...
    if ($result->num_rows > 0) {
        ...
        // Отримуємо дерево еволюції покемона - викликаємо в будь-якому випадку при зміні покемона
        $evolutionTree = getEvolutionTree($conn, $pokemon['id']);
        $pokemon['evolution_tree'] = $evolutionTree;
        
        // Отримуємо інформацію про еволюцію для сумісності зі старим кодом
        ...
        if ($result->num_rows > 0) {
            ...
            // Отримуємо дерево еволюції покемона - також викликаємо при циклічному переході
            $evolutionTree = getEvolutionTree($conn, $pokemon['id']);
            $pokemon['evolution_tree'] = $evolutionTree;
...
// Окремий ендпоінт для отримання еволюційного дерева
if (isset($_POST['action']) && $_POST['action'] == 'getEvolution') {
    $pokemonId = $_POST['pokemonId'];
    $evolutionTree = getEvolutionTree($conn, $pokemonId);
    
    // Повертаємо результат
    echo json_encode(['evolution_tree' => $evolutionTree]);
    $conn->close();
    exit;
}
...
if ($result->num_rows > 0) {
    ...
    // Отримуємо дерево еволюції покемона при ініціалізації
    $evolutionTree = getEvolutionTree($conn, $initialPokemon['id']);
    $initialPokemon['evolution_tree'] = $evolutionTree;
    
    // Отримуємо еволюційний ланцюг для сумісності зі старим кодом
    ...
    $initialPokemon = [
        ...
        'evolution_tree' => '<strong>Starting Pokémon:</strong> Pichu (ID=172) <br><br>
...
<body>
        ...
        <div class="pantalla first-level-parax">
          ...
          <div class="topbar">
            ...
            <div>
              ...
              <div id="type2" class="type2" style="dispay">Electric</div>
            </div>
          </div>
          ...
          <section class="screen third-screen">
            <div class="img-height-weight">
              ...
              <p class="height"><span>}</span><strong id="height">0.4</strong> m</p>
              <p class="weight"><strong id="weight">6.0</strong> kg</p>
            </div>
            <div class="gender">
              <p>M</p>
              <p>F</p>
          ...
          <section class="screen fourth-screen">
            ...
            <p id="evolution-text" class="evolution-text">
              <!-- Тут буде відображатися результат виклику процедури evolution_tree_pokemon -->
            </p>
          </section>
    ...
    <script>
      ...
      // Встановлюємо еволюцію при завантаженні сторінки
      document.addEventListener('DOMContentLoaded', function() {
        // Показуємо дерево еволюції для початкового покемона
        if (initialPokemon.evolution_tree) {
          document.getElementById('evolution-text').innerHTML = initialPokemon.evolution_tree;
        }
        
        // Асинхронний запит для оновлення дерева еволюції при зміні покемона
        window.updateEvolutionTree = function(pokemonId) {
          fetch('index.php', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded',
            },
            body: 'action=getEvolution&pokemonId=' + pokemonId
          })
          .then(response => response.json())
          .then(data => {
            if (data.evolution_tree) {
              document.getElementById('evolution-text').innerHTML = data.evolution_tree;
            }
          })
